...
var Events = {
  ...
  init: function (options) {
    ...
    Events.EventPool = [].concat(Events.Global, Events.Room, Events.Outside);
  ...
  createAttackButton: function (weaponName) {
    ...
    for (var k in weapon.cost) {
      if (
        typeof Path.outfit[k] != "number" ||
        Path.outfit[k] < weapon.cost[k]
      ) {
        Button.setDisabled(btn, true);
        break;
      }
    ...
    if (Events.activeEvent()) {
      ...
      if (weapon.type == "unarmed") {
        ...
        if ($SM.get("character.punches") == 50 && !$SM.hasPerk("boxer")) {
          ...
          $SM.get("character.punches") == 150 &&
          !$SM.hasPerk("martial artist")
        ) {
          $SM.addPerk("martial artist");
        } else if (
          $SM.get("character.punches") == 300 &&
      ...
      if (weapon.cost) {
        ...
        if (out) {
          ...
          $(".weaponButton").each(function () {
            if (
              !Button.isDisabled($(this)) &&
              $(this).attr("id") != "attack_fists"
            ) {
              validWeapons = true;
              return false;
            }
          ...
          if (!validWeapons) {
            ...
            if (fists.length === 0) {
              Events.createAttackButton("fists").prependTo(
                "#buttons",
                Events.eventPanel()
              );
            } else {
      ...
      var attackFn =
        weapon.type == "ranged" ? Events.animateRanged : Events.animateMelee;
    ...
    if (typeof dmg == "number") {
      if (dmg < 0) {
        ...
        if (meditating) {
          ...
          enemyHp = Math.min(
            maxHp,
            Math.max(0, enemyHp + (shielded ? dmg : -dmg))
          );
  ...
  setTakeAll: function (lootButtons) {
    ...
    lootButtons.children(".lootRow").each(function (i) {
      ...
      var num = Math.min(
        Math.floor(Path.getFreeSpace() / Path.getWeight(name)),
        numLeft
      );
  ...
  canLeave: function (btn) {
    var basetext = btn.data("canTakeEverything")
      ? _("take everything")
      : _("take all you can");
    var textbox = btn.children("span");
    var takeAndLeave = btn.data("leaveBtn")
      ? btn.data("canTakeEverything")
      : false;
    var text = _(basetext);
    if (takeAndLeave) {
      Button.cooldown(btn);
  ...
  buttonClick: function (btn) {
    var info =
      Events.activeEvent().scenes[Events.activeScene].buttons[btn.attr("id")];
    // Cost
    var costMod = {};
    if (info.cost) {
      const cost = {
    ...
    if (info.nextEvent) {
      const eventData =
        Events.Setpieces[info.nextEvent] || Events.Executioner[info.nextEvent];
  ...
  startEvent: function (event, options) {
    ...
    var currentSceneInformation =
      Events.activeEvent().scenes[Events.activeScene];
  ...
  scheduleNextEvent: function (scale) {
    var nextEvent =
      Math.floor(
        Math.random() *
          (Events._EVENT_TIME_RANGE[1] - Events._EVENT_TIME_RANGE[0])
      ) + Events._EVENT_TIME_RANGE[0];
    if (scale > 0) {
      nextEvent *= scale;
    }
    Engine.log("next event scheduled in " + nextEvent + " minutes");
    Events._eventTimeout = Engine.setTimeout(
      Events.triggerEvent,
      nextEvent * 60 * 1000
    );
  },
  ...
  handleStateUpdates: function (e) {
    if (
      (e.category == "stores" || e.category == "income") &&
